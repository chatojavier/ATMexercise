...
function NumberButton({ value, color, onClickNumber, colSpan, onClickAccept, submitDisabled }) {
    return (
        <button className={"number-button p-3 font-bold text-xl rounded-xl border border-solid text-center active:bg-gradient-to-br active:text-white" + (submitDisabled ? " opacity-30 cursor-default" : " hover:shadow") + (color ? ` border-${color} text-${color} from-${color} to-${color}-dark` : " border-blue text-blue from-blue to-blue-dark") + (colSpan ? ` col-span-${colSpan}` : "")} onClick={onClickAccept ? onClickAccept : onClickNumber} disabled={submitDisabled}>
            {value}
        </button>
    )
}